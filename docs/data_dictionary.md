...
| Tabela | Papel |
|---|---|
| `instruments` | Cadastro mestre = **identidade**. `ticker` **não** é identificador eterno — toda FK usa `instrument_id`. `company_id` (FK nullable) liga o ticker à companhia emissora. **`active` = escopo operacional dos pipelines, NÃO vigência histórica** — `active=true` é o filtro de `sync-prices --all`, `sync-news --all`, `fundamentals_ingest`, `audit` e os `--all` de `compute-*`; PETR3/ITUB3 são negociados hoje e estão `active=false` por decisão de escopo. Vigência histórica é respondida por `instrument_lifecycle` (fonte única); `instruments.valid_from`/`valid_to` permanecem NULL de propósito. |
...
| `instrument_lifecycle` | Ciclo de vida do instrumento/classe (Fase 3 M1). `valid_from`/`valid_to`/`listing_start`/`listing_end` = tempo efetivo (negociação real, da CVM FCA). `ticker` (`Codigo_Negociacao`) **nullable — NULL antes de 2018** (a FCA não informa; `quality_flag='incomplete'`) e **texto livre quando presente** (82 linhas trazem `000000`/`NÃO HÁ`/`N/A` — valor bruto preservado, `quality_flag='inconsistent'`); `share_class` + `company_id` discriminam. `source_*` = tempo de transação, nunca gate do universo estrutural. `source_reference_year_first` (M2) = menor ano de FCA que reportou a identidade — usado **só na camada investível** para separar `resolved` de `back_projected`. Fonte `cvm_fca` + `seed_manual` (classes históricas que a FCA não lista, ex.: VALE5). Ver `docs/historical_universe.md` §7 + `fase3_handoff_v2.md` §4.2. *(migrações `20260830044442`, `20260830055948`)* |
| `liquidity_metrics` | Liquidez point-in-time por instrumento/data (Fase 3 M2). Métrica **de mercado**, deliberadamente fora de `fundamental_metrics` (que é chaveada por documento CVM com `available_from` — liquidez não tem essa semântica e forçá-la fabricaria um gate falso). Volume financeiro = **`close` bruto × volume**, nunca `adj_close` (recalculado retroativamente a partir de proventos/splits futuros = look-ahead). Janelas de 20/60 **pregões** via `trading_calendar`, só `trade_date <= as_of_date`. Guarda o denominador esperado (`expected_trading_days_*`) e a mediana de 60 (cauda pesada: um leilão isolado infla a média). Ver `docs/historical_universe.md` §8. *(migração `20260830060954`)* |
| `instrument_price_window` | Janela canônica `[price_valid_from, price_valid_to]` por instrumento (Fase 3 M2.1). **Derivada e recomputável** — não é fonte de verdade (vigência efetiva = `instrument_lifecycle`). `price_valid_from = max(company start, class listing_start, 01/01/source_reference_year_first)`, salvo exceção de continuidade independente (`config/price_continuity_exceptions.yaml`). Linha do provedor fora desta janela **não entra** em `daily_prices` (`ticker_identity_not_proven`). `from_precision` (`day`/`year`/`unknown`) registra a precisão do limite; `to_precision` (`day`/`year`/`open`). Caso B: variante de nome truncada em `date(year_first_sucessor−1, 12, 31)`. *(migração `20260830151258`)* |
| `price_backfill_runs` / `price_backfill_attempts` | Ledger do backfill histórico de preços (Fase 3 M2.1). Uma linha de `attempts` por `(backfill_run, instrumento)` — checkpoint/resume; nenhum instrumento some entre etapas (status + motivo sempre). `rows_written` = só o que caiu **dentro** da janela canônica; `rows_out_of_window` fica no bruto/ledger. `status ∈ {dry_run, resolved, empty_series, symbol_not_found, failed, skipped_out_of_scope, ...}`; `checks` (jsonb) = resultado dos 8 checks. *(migração `20260830151747`)* |
