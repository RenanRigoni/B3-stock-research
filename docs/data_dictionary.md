...
| `wacc_assumptions` | WACC por companhia/`as_of_date` — decomposição completa do §21.6 (risk-free, beta, cost of equity, cost of debt, pesos). `inputs` (jsonb). *(migration `..06`)* |
| `valuation_snapshots` | Resultado de cada método de valor justo por `(companhia, as_of, método, cenário)`. V1: `valuation_method='fcff'`, cenários pessimista/base/otimista, `fair_value_per_share`, `margin_of_safety`, `assumptions` (jsonb). Não sobrescreve silenciosamente. Ver `fase2_plan.md` §10/§11/§12. *(migration `..06`)* |
